...
var URL = Class(function() {
  this._extractionRegex = new RegExp(['^', '((\\w+:)?//)?', '(\\w[\\w\\.\\-]+)?', '(:\\d+)?', '(\\/[^\\?#]+)?', '(\\?[^#]+)?', '(#.*)?'].join(''), 'i');
  ...
  this.toString = function() {
    return [this.protocol || '//', this.host, this.pathname, this.getSearch(), this.getHash()].join('');
  };
  ...
  this.getSearch = function() {
    return (this._searchParams ? '?' + url.query.string(this._searchParams) : this.search ? '?' + this.search : '');
  };
  this.getHash = function() {
    return (this._hashParams ? '#' + url.query.string(this._hashParams) : this.hash ? '?' + this.hash : '');
  };
